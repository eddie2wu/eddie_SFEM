function results = run_step_one(treatment, perfect_quiz)
    ...
    config.output_mat_file = sprintf('result/step_one_fix_sigma_results_t%d.mat', treatment);
    config.Gamma = [32, 32, 32, 32, 32, 32, 32, 32];
    config.max_outer_iter = 200;
    config.outer_tol = 1e-6;

    % number of random starts
    config.num_starts = 50;
    ...
    all_nll = Inf(config.num_starts, 1);
    all_seeds = NaN(config.num_starts, 1);
    all_results = cell(config.num_starts, 1);
    
    parfor s = 1:config.num_starts
        ...
        try
            ...
            results_s = estimate_step_one_fix_sigma(data, config);
    
            all_results{s} = results_s;
    ...
    % ---------------------------------
    % Find best result AFTER parfor
    % ---------------------------------
    
    [best_nll, idx_best] = min(all_nll);
    
    best_results = all_results{idx_best};
    best_start = idx_best;
    best_seed = all_seeds(idx_best);
