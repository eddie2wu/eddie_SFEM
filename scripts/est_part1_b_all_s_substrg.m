...
for s = 1:length(suffixes)
    ...
    parfor filenum = 1:8
        ...
        disp('alpha = 0');
        [h_values(1), p_values(1)] = waldtest(x(1), [1 0 0], vc_bs);
    
        disp('p(1) = 0');
        [h_values(2), p_values(2)] = waldtest(x(2), [0 1 0], vc_bs);
    
        disp('p(2) = 0');
        [h_values(3), p_values(3)] = waldtest(x(3), [0 0 1], vc_bs);
    
        disp('p(3) = 0');
        [h_values(4), p_values(4)] = waldtest(sum(x(2:3))-1, [0 1 1], vc_bs);
        ...
        % Save the results
        output_filename = sprintf('raw_substrg/est_part1_b_%d_s%s.mat', filenum, suffix);
